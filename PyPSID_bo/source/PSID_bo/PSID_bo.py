...
def PSID_bo(Y, Z=None, nx=None, n1=0, i=None, WS=dict(), return_WS=False, fit_Cz_via_KF=True, time_first=True):
    """
    PSID_bo: Preferential Subspace Identification Algorithm
    ...
    if n1 > 0 and nz > 0:
        ...
        if 'ZHat_U' not in WS or WS['ZHat_U'] is None:
            ...
            #REPLACE PROJECTION WITH BEHAVIOUR
            WS['ZHat'] = Zf #projOrth(Zf, WS['Yp'])[0] # Zf @ WS['Yp'].T @ np.linalg.pinv(WS['Yp'] @ WS['Yp'].T) @ WS['Yp']  # Eq. (10)
            Yp_Plus = np.concatenate((WS['Yp'], WS['Yii']))
            Zf_Minus = Zf[nz:, :]
            #REPLACE PROJECTION WITH BEHAVIOUR
            WS['ZHatMinus'] = Zf_Minus #projOrth(Zf_Minus, Yp_Plus)[0] # Zf_Minus @ Yp_Plus.T @ np.linalg.pinv(Yp_Plus @ Yp_Plus.T) @ Yp_Plus  # Eq. (11)           
            # Take SVD of ZHat
    ...
    Cy_prime = projOrth(WS['Yii'], WS['Zii'])[1] # WS['Yii'] @ Xk.T @ np.linalg.pinv(Xk @ Xk.T)    # Eq. (32)
